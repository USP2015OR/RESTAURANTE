...
require_once("conexion.php");
	...
	$passvieja=$_POST['txtpassantigua'];
        ...
        if($passnueva1==$passnueva2){
            $query="SELECT * FROM `usuario` WHERE `uduario_id`=".$id." and `usuario_contrasena`=md5('".$passvieja."')";
            $rs=mysql_query($query,$cnn);
            $n=is_resource($rs)?mysql_num_rows($rs):0;
            if($n>0){
                $query2="update usuario set usuario_estado=1,`usuario_contrasena`=md5('".$passnueva1."') where uduario_id='".$id."'";
                //echo $query2;
